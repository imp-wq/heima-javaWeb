...
/*
1. 部门编号、部门名称、部门位置从dept表种查询。
2. 部门人数从emp表中，按dept_id分组，使用count统计。
3. 将以上2表进行left join。
*/

SELECT id, dname, loc FROM dept;
-- 给count(*)聚合函数起别名
SELECT dept_id,COUNT(*) count FROM emp GROUP BY dept_id;
SELECT t1.id,t1.dname,t1.loc,t2.count FROM (SELECT id, dname, loc FROM dept) t1 JOIN (SELECT dept_id,COUNT(*) count FROM emp GROUP BY dept_id) t2 ON t1.id=t2.dept_id;
